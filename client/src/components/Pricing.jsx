  ...
  return (
    <section className="py-24 bg-white dark:bg-background">
      <div className="container">
        ...
        <div
          className="flex flex-col gap-6 items-center mt-12 lg:flex-row lg:items-end lg:justify-center">
          {pricingTiers.map(({ title, monthlyPrice, buttonText, popular, features, inverse }) => (
            <Card
              ...
              <CardHeader className="flex justify-between items-start">
                <CardTitle
                  className={`text-lg font-bold ${inverse ? "text-white/70" : "text-muted-foreground"}`}>
                  {title}
                </CardTitle>
                {popular && (
                  <motion.div
              ...
              <CardContent>
                <div className="flex items-baseline gap-1 mt-2">
                  ...
                  <span
                    className={`tracking-tight font-semibold ${inverse ? "text-white/60" : "text-muted-foreground"}`}>
                    /month
                  </span>
                </div>
                <Button variant={inverse ? "secondary" : "default"} className="cursor-pointer w-full mt-6">
                  {buttonText}
                </Button>
